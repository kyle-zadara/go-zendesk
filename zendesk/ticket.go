...
func (c *client) ListRequestedTicketsFull(userID int64, options *ListOptions, sideloads ...SideLoad) (*ListResponse, error) {
	params, err := query.Values(options)
	if err != nil {
		return nil, err
	}
	...
	out := new(APIPayload)
	err = c.get(fmt.Sprintf("/api/v2/users/%d/tickets/requested.json?%s", userID, params.Encode()), out)
